...
import { BarChart, Bar, YAxis, XAxis, Tooltip, ResponsiveContainer,Cell,Text } from 'recharts';
          ...
          return (
            <div style={{ display: 'flex' }}>
                <ResponsiveContainer width="70%" height={160} >
                    ...
                    <Tooltip content={<CustomTooltip />} cursor={false} wrapperStyle={{ background: 'transparent' }} />
                    <Bar dataKey="count" fill="#F89446" barSize={10} barRadius={5} />
                    </BarChart>
                </ResponsiveContainer>

                <Box style={{width: '30%', display: 'flex', alignItems: 'center', flexDirection: 'column', marginLeft:'10px' }}>
                    <Typography style={{ fontSize: '4.5rem', color: crAppTheme.palette.primary.dark}}>{averageRating}</Typography>
                    <Rating size="small" value={averageRating} readOnly />
                    <Typography variant="caption" sx={{ color: crAppTheme.palette.primary.dark }}>{`(${ratings.length})`}</Typography>
                </Box>
                </div>
          );
      };
